...
export const NETWORK_CONFIG_QUESTIONS: { [key in keyof NETWORK_CONFIG]: DistinctQuestion & { name: key } } = {
  AUTOTASK_ID: {
    name: "AUTOTASK_ID",
    type: "input",
 ...
 */
export const populateConfig = async <G extends Record<string, any> = {}, N extends Record<string, any> = {}>(config: Configstore,
  {
    askFlashbots,
    extraConfig
  }: {
  ...
  const globalQuestions = [...Object.values(GLOBAL_CONFIG_QUESTIONS),  ...(extraConfig?.global ?? [])];
  const networkQuestions = [...Object.values(NETWORK_CONFIG_QUESTIONS), ...(extraConfig?.network ?? [])];
  ...
  const previousNetworkName = previousNetwork ? Object.fromEntries(Object.entries(CHAIN_IDS).map(([name, id]) => [id, name]))[previousNetwork] : null;
  const { CHAIN_ID } = await inquirer.prompt({
    name: "CHAIN_ID",
    type: "list",
    message: chalk.green("Which network?"),
    choices: [...(previousNetworkName ? [`Last Used (${previousNetworkName})`] : []), "Mumbai", "Goerli", "Sepolia", "Mainnet"],
    filter(val: string) {
  ...
  if(configHasKeys(config, globalKeys)) {
    const { USE_GLOBAL_CONFIG } = await inquirer.prompt({
      ...
      type: "list",
      message: chalk.green(`Would you like to use the existing OZ Defender config? (${config.path})`),
      choices: ["Yes", "No"],
      filter(val: string) {
